...
$('.btn-weixin-img').click(function () {
    ...
    $("body").append('<div class="mobile-share-bg"><div class="top_tips">Please press the picture to recommend the content to your friends.</div></div><div class="mobile-share-wrap"></div>'),
        popup.showToast({
            type: "icon",
            time: 1e5
        }),
            ...
            success: function (t) {
                ...
                t.qrcode = o.find("canvas")[0].toDataURL(),
                    t.head && t.logo && t.qrcode ? utils.textToImage(t) : (popup.showToast({
                        type: "text",
                        text: "Get sharing pictures failed！"
                    }), $(".mobile-share-bg,.mobile-share-wrap").remove())
            },
            error: function () {
                popup.showToast({
                        ...
                        success: function (data) {
                            ...
                            $(".read-clean").bind("click", function () {
                                if (_varKr.body.hasClass("is-read")) {
                                    ...
                                    $(".read-clean i").removeClass("icon2-read2").addClass("icon2-read").attr("title", "Reading mode");
                                } else {
                                    _varKr.body.addClass("is-read");
                                    $(".read-clean i").removeClass("icon2-read").addClass("icon2-read2").attr("title", "Reading mode");
                                }
                                $("html, body").animate({
                                        scrollTop: 0
                                    },
...
if ($('body').hasClass("single")) {
        ...
        if (pageCount <= pages) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (pageCount < pages) {
                        $containerList.next('.loader-trigger').after('<div class="more-trigger"><a href="javascript:;" class="button">Show more comments</a></div>');
                    }
    ...
    $(".must-login").click(function () {
        $(".post-comments-wrap").find("#comment-content").after("<p style='color:red;font-size:12px;display:none'>please log in first</p>").next().show(500).delay(2000).fadeOut(1000, function () {
            $(this).remove();
        });
        return;
    });
    ...
    $(".post-comments-wrap .form-submit #submit").bind("click", function () {
        ...
        if (content.length < 2) {
            commcon.find("#comment-content").after("<p style='color:red;font-size:12px;display:none'>This point is too short！</p>").next().show(500).delay(2000).fadeOut(1000, function () {
                $(this).remove();
            });
            return false;
        }
        ...
        $.post(
            ...
            function (data) {
                ...
                if (data == 'captcha_error') {
                    commcon.find("#comment-content").after("<p style='color:red;font-size:12px;display:none'>Verification code error！</p>").next().show(500).delay(2000).fadeOut(1000, function () {
                        $(this).remove();
                    });
                    return false;
                } else if (data == 'content_error') {
                    commcon.find("#comment-content").after("<p style='color:red;font-size:12px;display:none'>Please enter a comment</p>").next().show(500).delay(2000).fadeOut(1000, function () {
                        $(this).remove();
                    });
                    return false;
                } else if (data == 'email_error') {
                    commcon.find("#comment-content").after("<p style='color:red;font-size:12px;display:none'>Please enter a valid mailbox！</p>").next().show(500).delay(2000).fadeOut(1000, function () {
                        $(this).remove();
                    });
                    return false;
                } else if (data == 'login_error') {
                    commcon.find("#comment-content").after("<p style='color:red;font-size:12px;display:none'>please log in first！</p>").next().show(500).delay(2000).fadeOut(1000, function () {
                        $(this).remove();
                    });
                    return false;
                } else if (data == 'info_error') {
                    commcon.find("#comment-content").after("<p style='color:red;font-size:12px;display:none'>Please enter a nickname！</p>").next().show(500).delay(2000).fadeOut(1000, function () {
                        $(this).remove();
                    });
                    return false;
                } else {
